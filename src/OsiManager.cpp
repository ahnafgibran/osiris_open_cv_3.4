...
namespace osiris
{
    ...
    void OsiManager::run()
    {
        ...
        if (mProcessMatchingFromBuffer && mProcessMatching)
        {
            ...
            OsiEye eye;
            std::string subjectDir = "../data/Output/BufferIrisCodes";

            std::vector<std::string> irisFiles;
            for (const auto &entry : fs::directory_iterator(subjectDir))
            {
                if (fs::is_regular_file(entry) && entry.path().extension() == ".bin")
                {
                    irisFiles.push_back(entry.path().string());
                }
            }

            for (size_t i = 0; i < irisFiles.size(); ++i)
            {
                for (size_t j = i + 1; j < irisFiles.size(); ++j)
                {
                    std::string outputFileName = "../data/Output/Matching/" + fs::path(irisFiles[i]).stem().string() + "_matching.txt";
                    ...
                    catch (const std::exception &e)
                    {
                        ...
                        throw std::runtime_error("Cannot create the file for matching scores : " + outputFileName);
                    }

                    try
                    {
                        std::chrono::steady_clock::time_point begin_matching = std::chrono::steady_clock::now();
                        float score = eye.matchFromBuffer(irisFiles[i], irisFiles[j], mpApplicationPoints);
                        ...
                        if (result_matching)
                        {
                            ...
                            try
                            {
                                result_matching << irisFiles[i] << " ";
                                result_matching << irisFiles[j] << " ";
                                result_matching << score << " ";
                                result_matching << std::chrono::duration_cast<std::chrono::milliseconds>(end_matching - begin_matching).count() << "[ms]" << std::endl;
                            }
                            catch (const std::exception &e)
                            {
                                std::cout << e.what() << std::endl;
                                throw std::runtime_error("Error while saving result of matching in " + mOutputFileMatchingScores);
                            }
                        }
                    }
                    catch (const std::exception &e)
                    {
                        ...
                        if (result_matching)
                        {
                            ...
                            {
                                std::cout << e.what() << std::endl;
                                throw std::runtime_error("Error while saving result of matching in " + outputFileName);
                            }
                        }
                    }
                }
        ...
        else
        {
            ...
            errorFile.open("../data/Output/error.txt", std::ios_base::app);
                        ...
                        if (fs::is_regular_file(imageEntry))
                        {
                            ...
                            if (extension == "jpg" || extension == "bmp")
                            {
                                try
                                {
                                    // Process the eye
                                    OsiEye eye;
                                    processOneEye(irisImagePath, eye);
                                }
                                catch (exception &e)
                                {
                                    // save the error to a file with the irisImage Path

                                    errorFile << "Error processing image: " << irisImagePath << " ";
